use super::collision::{check_collision_with_world, AABB};
...
fn ensure_player_has_physics(
    ...
    for (entity, transform) in query.iter() {
        ...
        physics.aabb.center =
            transform.translation - Vec3::new(0.0, PLAYER_EYE_HEIGHT - PLAYER_HEIGHT / 2.0, 0.0);
        let aabb_center = physics.aabb.center;

        commands.entity(entity).insert(physics);
        info!(
            "Added PlayerPhysics to camera at position: {:?} (AABB center: {:?})",
    ...
    if velocity.length_squared() > 0.0 {
        ...
        unsafe {
            ...
            if current - LAST_LOG > 1.0 {
                info!(
                    "Simple movement working! Position: {:?}",
...
pub fn update_player_physics(
    ...
    keyboard: Res<ButtonInput<KeyCode>>,
    mut query: Query<
        (&mut Transform, &mut CameraController, &mut PlayerPhysics),
        With<PlayerCamera>,
    >,
    ...
    unsafe {
        if FIRST_FRAME {
            info!(
                "PHYSICS FIRST FRAME: Position: {:?}, chunks available: {}",
                transform.translation,
...
fn apply_player_movement(
    ...
    if controller.fly_mode {
        ...
        transform.translation.y = transform
            .translation
            .y
            .min(crate::planet::config::MAX_ALTITUDE);
        return;
    }
    ...
    if desired_movement.y <= 0.0 {
        ...
        if !physics.is_grounded || desired_movement.y < -0.1 {
            ...
            if !check_collision_with_world(&test_aabb, &chunk_query) {
                ...
                for _ in 0..15 {
                    // 15 iterations for better precision
    ...
    if physics.is_grounded && (desired_movement.x != 0.0 || desired_movement.z != 0.0) {
        ...
        if horizontal_blocked {
            ...
            for step_height in [0.1, 0.25, 0.5, STEP_HEIGHT] {
                let step_pos =
                    new_position + Vec3::new(desired_movement.x, step_height, desired_movement.z);
                ...
                if !check_collision_with_world(&test_aabb, &chunk_query) {
                    ...
                    if check_collision_with_world(&ground_check, &chunk_query) {
                        ...
                        for down_check in 1..=5 {
                            ...
                            let check_pos = new_position
                                + Vec3::new(desired_movement.x, check_y, desired_movement.z);
    ...
    transform.translation =
        new_position + Vec3::new(0.0, PLAYER_EYE_HEIGHT - PLAYER_HEIGHT / 2.0, 0.0);
    transform.translation.y = transform
        .translation
        .y
        .min(crate::planet::config::MAX_ALTITUDE);
