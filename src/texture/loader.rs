            ...
            if let Ok(metadata) = entry.metadata() {
                if metadata.is_dir() {
                    if let Some(block_name) = entry.file_name().to_str() {
                        ...
                        let block_textures =
                            scan_block_directory(&entry.path(), block_name.to_string());
...
fn scan_block_directory(path: &Path, name: String) -> BlockTextures {
    ...
    let mut parsed_textures: HashMap<(String, Option<String>, Option<usize>), PathBuf> =
        HashMap::new();
    ...
    for block in blocks {
        for ((face, state), paths) in block.faces {
            ...
            if !frame_indices.is_empty() {
                ...
                let uv_min = Vec2::new(
                    (grid_x * (TEXTURE_SIZE + PADDING * 2) as f32 + PADDING as f32)
                        / atlas_size as f32,
                    (grid_y * (TEXTURE_SIZE + PADDING * 2) as f32 + PADDING as f32)
                        / atlas_size as f32,
                );
                let uv_max = Vec2::new(
