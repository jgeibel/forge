...
impl ChunkLighting {
    ...
    pub fn update(&mut self, game_time: &GameTime) {
        ...
        self.sun_angle = calculate_local_sun_angle(chunk_center_x, chunk_center_z, game_time);
        ...
        if self.sun_angle <= -0.2 {
            ...
            self.sky_color = Color::srgb(0.05 + 0.45 * t, 0.05 + 0.25 * t, 0.15 + 0.35 * t);
            self.fog_color = Color::srgb(0.02 + 0.48 * t, 0.02 + 0.28 * t, 0.05 + 0.35 * t);
            ...
            self.fog_color = Color::srgb(0.5 + 0.2 * (1.0 - t), 0.3 + 0.5 * t, 0.4 + 0.5 * t);
        } else {
            // Full daylight
...
impl Plugin for LightingPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<LightingUpdateTimer>().add_systems(
            Update,
            (update_chunk_lighting, apply_player_lighting).chain(),
        );
    }
}
