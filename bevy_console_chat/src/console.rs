...
/// Event for chat messages (non-command input)
#[derive(Event)]
...
pub struct ConsoleOpen {
    /// Console open
    pub open: bool,
    /// Pre-populate with slash when opening
    pub open_with_slash: bool,
}
...
pub(crate) fn console_ui(
    ...
    mut chat_messages: EventWriter<ConsoleChatMessage>,
                ...
                ui.vertical(|ui| {
                    ...
                    ScrollArea::vertical()
                        ...
                        .show(ui, |ui| {
                            ...
                            if console_open.is_changed() {
                                ...
                                // Pre-populate with slash if requested
                                if console_open.open_with_slash && state.buf.is_empty() {
                                    state.buf = "/".to_string();
                        ...
                        && ui.input(|i| i.key_pressed(egui::Key::Enter))
                    {
                        if state.buf.trim().is_empty() {
                            ...
                            if state.buf.starts_with('/') {
                                // Remove the leading slash and process as command
                                let command_input = &state.buf[1..];
                                let mut args = Shlex::new(command_input).collect::<Vec<_>>();
                                ...
                                // Not a command, emit as chat message
                                chat_messages.send(ConsoleChatMessage {
                                    message: state.buf.clone(),
                                });
                            }
